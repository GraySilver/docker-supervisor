...
from docker_supervisor.tests.base import PopulatedDummySupervisor
from docker_supervisor.tests.base import _NOW
from docker_supervisor.tests.base import _TIMEFORMAT
...
class SupervisorNamespaceXMLRPCInterfaceTests(TestBase):
    def _getTargetClass(self):
        from docker_supervisor import rpcinterface
    ...
    def test_update(self):
        from docker_supervisor import xmlrpc
        from docker_supervisor.supervisord import SupervisorStates
    ...
    def test_getSupervisorVersion(self):
        ...
        from docker_supervisor import options
    ...
    def test_getState(self):
        from docker_supervisor.states import getSupervisorStateDescription
        supervisord = DummySupervisor()
        interface = self._makeOne(supervisord)
    ...
    def test_addProcessGroup(self):
        from docker_supervisor.supervisord import Supervisor
        from docker_supervisor import xmlrpc
        options = DummyOptions()
    ...
    def test_startProcessGroup(self):
        ...
        supervisord = PopulatedDummySupervisor(options, 'foo', pconfig1,
                                               pconfig2)
        from docker_supervisor.process import ProcessStates
        from docker_supervisor.xmlrpc import Faults
        supervisord.set_procattr('process1', 'state', ProcessStates.STOPPED)
        supervisord.set_procattr('process2', 'state', ProcessStates.STOPPED)
        interface = self._makeOne(supervisord)
...
class Test_make_main_rpcinterface(unittest.TestCase):
    def _callFUT(self, supervisord):
        from docker_supervisor.rpcinterface import make_main_rpcinterface
