...
from docker_supervisor.tests.base import DummyPGroupConfig
from docker_supervisor.tests.base import DummyDispatcher
from docker_supervisor.tests.base import DummyEvent
from docker_supervisor.tests.base import DummyFCGIGroupConfig
from docker_supervisor.tests.base import DummySocketConfig
from docker_supervisor.tests.base import DummyProcessGroup
from docker_supervisor.tests.base import DummyFCGIProcessGroup

from docker_supervisor.process import Subprocess
from docker_supervisor.options import BadCommand
...
class SubprocessTests(unittest.TestCase):
    ...
    def test_getProcessStateDescription(self):
        ...
        from docker_supervisor.process import getProcessStateDescription
    ...
    def _dont_test_spawn_and_kill(self):
        ...
        from docker_supervisor.tests.base import makeSpew
        try:
            called = 0
            def foo(*args):
                called = 1
    ...
    def test_transition_stopped_to_starting_supervisor_stopping(self):
        ...
        L = []
        events.subscribe(events.ProcessStateEvent, lambda x: L.append(x))
        from docker_supervisor.states import ProcessStates, SupervisorStates
        options = DummyOptions()
    ...
    def test_transition_exited_to_starting_supervisor_stopping(self):
        ...
        # this should not be spawned, as supervisor is shutting down
        pconfig = DummyPConfig(options, 'process', 'process','/bin/process')
        from docker_supervisor.datatypes import RestartUnconditionally
    ...
    def test_transition_exited_to_starting_condit_supervisor_running(self):
        ...
        pconfig = DummyPConfig(options, 'process', 'process','/bin/process')
        from docker_supervisor.datatypes import RestartWhenExitUnexpected
...
class ProcessGroupBaseTests(unittest.TestCase):
    def _getTargetClass(self):
        from docker_supervisor.process import ProcessGroupBase
...
class EventListenerPoolTests(ProcessGroupBaseTests):
    ...
    def test__acceptEvent_attaches_pool_serial_and_serial(self):
        from docker_supervisor.process import GlobalSerial
        options = DummyOptions()
        gconfig = DummyPGroupConfig(options)
        pconfig1 = DummyPConfig(options, 'process1', 'process1','/bin/process1')
        gconfig = DummyPGroupConfig(options, pconfigs=[pconfig1])
...
class test_new_serial(unittest.TestCase):
    def _callFUT(self, inst):
        from docker_supervisor.process import new_serial
