...
class ChildUtilsTests(unittest.TestCase):
    def test_getRPCInterface(self):
        from docker_supervisor.childutils import getRPCInterface
    ...
    def test_getRPCTransport_no_uname_pass(self):
        from docker_supervisor.childutils import getRPCTransport
    ...
    def test_get_headers(self):
        from docker_supervisor.childutils import get_headers
        line = 'a:1 b:2'
        result = get_headers(line)
        self.assertEqual(result, {'a':'1', 'b':'2'})

    def test_eventdata(self):
        from docker_supervisor.childutils import eventdata
        payload = 'a:1 b:2\nthedata\n'
    ...
    def test_get_asctime(self):
        from docker_supervisor.childutils import get_asctime
...
class TestProcessCommunicationsProtocol(unittest.TestCase):
    def test_send(self):
        from docker_supervisor.childutils import pcomm
        stdout = StringIO()
...
class TestEventListenerProtocol(unittest.TestCase):
    def test_wait(self):
        from docker_supervisor.childutils import listener
        class Dummy:
            def readline(self):
                return 'len:5'
